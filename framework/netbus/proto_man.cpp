...
#define CMD_HEADER 8
	...
	// 只解析head(stype,ctype,utag) 不解析body
	...
	bool proto_man::decode_cmd_msg(unsigned char* cmd, int cmd_len, struct cmd_msg** out_msg) {
		...
		*out_msg = msg;
		...
		if (g_proto_type == PROTO_JSON) {
			...
			char* json_str = (char*)cache_manager::cache_alloc(wbuf_allocer, json_len + 1);
	...
	void proto_man::cmd_msg_free(struct cmd_msg* msg) {
		if (msg->body) {
			if (g_proto_type == PROTO_JSON) {
				// free(msg->body);
				cache_manager::cache_free(wbuf_allocer, msg->body);
				msg->body = NULL;
			}
			else {
	...
	unsigned char* proto_man::encode_msg_to_raw(const struct cmd_msg* msg, int* out_len) {
		...
		*out_len = 0;
		...
		if (g_proto_type == PROTO_JSON) {
			...
			raw_data = (unsigned char*)cache_manager::cache_alloc(wbuf_allocer, CMD_HEADER + len);
			if (msg->body != NULL) {
			...
			*out_len = (len + CMD_HEADER);
			...
			raw_data = (unsigned char*)cache_manager::cache_alloc(wbuf_allocer, CMD_HEADER + pf_len);

			if (msg->body) {
				if (!p_m->SerializePartialToArray(raw_data + CMD_HEADER, pf_len)) {
					// free(raw_data);
					cache_manager::cache_free(wbuf_allocer, raw_data);
					return NULL;
				}
			}
	...
	void proto_man::msg_raw_free(unsigned char* raw) {
		cache_manager::cache_free(wbuf_allocer, raw);
	}
}
